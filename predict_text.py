# predict_text.py
import os, re, json, joblib, numpy as np
from gensim.models import Word2Vec, FastText
import xgboost as xgb
import spacy

ART_DIR = "./artifacts_phase2/"
...
def idf_weight(t): return 1.0 / (idf_map.get(t, idf_default))
...
def sentence_vec(tokens, kv, dim):
    ...
    for t in tokens:
        if t in kv:
            w = idf_weight(t)
            weights.append(w); vecs.append(kv[t])
    if not vecs: return np.zeros(dim, dtype=np.float32)
    vecs = np.array(vecs); weights = np.array(weights).reshape(-1,1)
...
def ner_features(raw):
    ...
    for e in doc.ents:
        if e.label_ in counts: counts[e.label_] += 1
    return np.array([total] + [counts[l] for l in labels], dtype=np.float32)

def symptom_counts(raw):
    toks = clean_text_simple(raw).split()
    cnt = sum(1 for w in toks if w in symptom_terms)
    uniq = len(set(w for w in toks if w in symptom_terms))
    return np.array([cnt, uniq], dtype=np.float32)
...
def features_from_text(raw):
    ...
    # NER + symptom
    v_ner = ner_features(raw).reshape(1,-1)
    v_sym = symptom_counts(raw).reshape(1,-1)
    # concat and scale
...
def predict_from_text(raw_text: str):
    Xs = features_from_text(raw_text)
    p = float(bst.predict(xgb.DMatrix(Xs))[0])
    label = int(p >= best_threshold)
    return {"probability": p, "label": label, "threshold": best_threshold}

if __name__ == "__main__":
    while True:
        txt = input("\nEnter patient/drug text (or 'quit'):\n")
        if txt.strip().lower() in ("quit","exit"): break
        print("Prediction:", predict_from_text(txt))
